...
import org.training.cockpits.constants.CloudstoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = CloudstoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(CloudstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(CloudstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						CloudstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/cloudstorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
